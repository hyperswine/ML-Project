...
def costFunction(theta, X, y, LRlambda):
    m = np.shape(y)[0]
    J = 0;
    jj = -np.multiply(y,np.log(sigmoid(np.dot(X,theta)))) - np.multiply((1-y),np.log(1-sigmoid(np.dot(X,theta))))                                                            
    J = 1/m*np.sum(jj) + LRlambda/(2*m)*np.sum(theta[1:]**2) #DONT regularize theta0 since its bias term
    return J
...
def gradFunction(theta, X, y, LRlambda):
    ...
    grad = np.insert(grad_tmp[1:] + LRlambda/m*theta[1:], 0, grad_tmp[0], axis=0)
    return grad.flatten()
